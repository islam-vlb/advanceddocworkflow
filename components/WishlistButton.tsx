...
export default function WishlistButton({ id, className = "" }: { id: number; className?: string }) {
  ...
  return (
    <button
      ...
      className={`flex items-center justify-center rounded-xl bg-white/90 backdrop-blur-sm shadow-lg min-w-[44px] min-h-[44px] transition-all duration-300 hover:scale-110 ${className}`}
    >
      <svg
        width="18"
        height="18"
